...
if ($send!='')
{
	...
	{ 
			@$bd->ejecutar("update fracciones set numero='$numero', titulo='$nombre',descrip='$detalle' where id=$id");
...
$busca=$bd->ejecutar("select * from  fracciones where id=$id");
...
<form action="upfracc.php?id=<?=$id?>" method="post">
<table width="624" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr>
    <td><table width="596" border="0" align="center" cellpadding="1" cellspacing="5">
      ...
      <tr>
        <td colspan="4" align="center" class="Verdana16Ngo"><strong>Modificar Fraccion</strong></td>
        </tr>
      <tr>
        <td colspan="2" class="Hevel16Gris"><strong>N&uacute;mero:</strong></td>
        <td colspan="2">
        <?  
$row=$bd->obtener_fila($busca,0);
  ?>
        <label for="numero"></label>
          <input name="numero" type="text" id="numero" style="width:50px" onKeyPress="return entero(event)" onKeyUp="activa()" value="<?=$row['numero']?>" size="5" />
          en decimal</td>
      </tr>
      <tr>
        <td width="100" colspan="2" class="Hevel16Gris"><strong>Titulo:</strong></td>
        <td width="483" colspan="2"><label for="nombre"></label>
          <input name="nombre" type="text" id="nombre" onMouseOver="activa()" onKeyPress="activa()" onKeyUp="activa()" value="<?=$row['titulo'];?>" size="80"/></td>
      </tr>
      <tr>
        <td colspan="2" class="Hevel16Gris"><strong>Detalle:</strong></td>
        <td colspan="2" class="Hevel16Gris"><label for="detalle"></label>
          <textarea name="detalle" rows="5" id="detalle" style="width:100%"><?=$row['descrip']?></textarea></td>
        </tr>
      <tr>
        <td align="center">&nbsp;</td>
        <td align="center">&nbsp;</td>
        <td align="center"><input type="button" name="cancel" id="cancel" value="Cancelar" onClick="document.location='fracciones.php';" /></td>
        <td align="center"><input type="submit" name="send" id="send" value="Guardar" onMouseOver="checa()" /></td>
      </tr>
    </table></td>
  </tr>
</table>
