...
<form method="get" action="agenda.php">
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td width="22%" height="23" bgcolor="#333333" style="padding:10px 2px 10px 10px"><label for="buscar"></label>
      ...
      <td width="10%" bgcolor="#333333" style="/* [disabled]padding:10px 2px; */"><input type="text" name="datepicker" id="datepicker"   size="12"  class="naranja121" onchange="cambia()" value="<?=$datepicker;?>" /> <script type="text/javascript"> 
...
//alert (div_alto1+" "+div_alto2+" "+div_alto3)
    ...
    <tr>
      <td width="22%" height="22" align="right" bgcolor="#333333" class="arte2" style="padding: 0 5px 0 0;">Usar Fecha:</td>
      <td bgcolor="#333333" class="arte2" style="/* [disabled]padding:10px 2px; */">
      <input type="radio" name="radio" id="Si" value="1" <? if ($radio==1){ echo 'checked="checked" '; }  ?> />
        <label for="Si">Si</label> <input name="radio" type="radio" id="no" value="0"  <? if ($radio!=1){ echo 'checked="checked" '; }  ?>/> <label for="no">No</label></td>
      <td width="68%" bgcolor="#333333" style="/* [disabled]padding:10px 2px; */">&nbsp;</td>
    </tr>
  </table>
  </form>
</div>
...
echo $kaso['nombre'];
	  ?> <? } ?>  
		...
		<?
		$radio=$_GET['radio'];
		if ($datepicker!='' and $radio==1) { $cad2=" and ((newsdate between '".$datepicker." 00:00:01' and '".$datepicker." 23:59:59') or (inicio < '".$datepicker." 00:00:01' and fin > '".$datepicker." 00:00:01' )) "; 
			$eventos=$bd->ejecutar("select * from mynews where 1 $cad $cad2 and id not in ($idsp) order by newsdate asc ");
		//echo "select * from mynews where 1 $cad $cad2 and id not in ($idsp) order by newsdate asc  ";
		} else {
		//echo "select * from mynews where 1 $cad and newsdate >= '$desde'   and id not in ($idsp) order by newsdate desc";
			$eventos=$bd->ejecutar("select * from mynews where 1 $cad  and newsdate >= '$desde'  and id not in ($idsp) order by newsdate asc "); }
		...
		while($row=$bd->obtener_fila($eventos,0)){
		 ...
		 ?>
                ...
                <tr>
        <td width="118" rowspan="2" class="negro12">
       <a href="veragenda.php?id=<?=$row['id']?>" >  <? if(file_exists($foto)){ ?> <img src="<?=$foto?>" width="68" height="42" /><? } else { ?>
        ...
        <td width="162" class="negro12"><span class="naranja121"><?=$row['descri']?></span></td>
        <td width="214" class="azul12"><?=$rows['nombre']?></td>
        <td width="79" rowspan="2" class="negro15"><? if ($hora!='00:00'){ echo $hora.' Hrs'; } ?> </td>
        <td width="6" rowspan="2">&nbsp;</td>
        </tr>
      <tr>
        <td class="negro12"><strong><?=$row['newstitle']?></strong></td>
        <td width="214" class="azul12">  
		<? $inicioxx=substr($row['inicio'],0,10);
			$finxx=substr($row['fin'],0,10);
