...
cal.getFirstDate();
    ...
    <input name="Botón" type="button" class="naranja121" value="Cambiar" onclick="cambiaFecha()" />
      ...
      <tr>
		...
		$eventos=$bd->ejecutar("select * from mynews where newsdate between '$desde' and '$hasta' and secc2=0 order by newsdate asc limit 6 "); $cuantos=$bd->num_rows($eventos);
      ...
      <tr>
       ...
       <tr>
		...
		$hoy=date("Y-m-d H:i:s") ; 
		$eventos=$bd->ejecutar("select * from mynews where  fin>='$tope' and inicio<='$hoy' and secc2=1  limit $limite");
		while($row=$bd->obtener_fila($eventos,0)){
			$tdatet=explode(" ",$row['newsdate']);
			$fecha=$tdatet[0];
